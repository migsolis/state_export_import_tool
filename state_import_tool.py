...
class StateImportTool():
    ...
    def read_csv(self, path):
        self.import_table = pd.read_csv(path)
        self.prep_import_table()
    
    def write_xml(self, path: str='', name: str='output') -> None:
        self.xml.write(f'{path}/{name}.xml')

    def prep_import_table(self):
        self.import_table.drop('Path', axis=1, inplace= True)
        self.import_table['Path'] = self.import_table['Parent'] + '/'+ self.import_table['Name']
        self.import_table.sort_values('Path', inplace=True)
        self.import_table['Error'] = ''

    def check_errors(self):         
        duplicate_codes = self.check_duplicate_state_codes(self.import_table)
        duplicate_paths = self.check_duplicate_paths(self.import_table)
        missing_parents = self.check_missing_parent(self.import_table)

        if duplicate_codes or duplicate_paths or missing_parents:
            self.generate_output_file(self.import_table, args.name)

    def generate_xml(self, import_table: pd.DataFrame, name: str) -> ET.ElementTree:
        ...
        index = self.create_tree(0, import_table, self.ROOT_CHAR, root)
        if index < import_table.shape[0]:
            self.update_error_column(import_table, 'Error Creating Node', [index])
            self.generate_output_file(import_table, name)
        ET.indent(root)
        ET.dump(root)
        self.xml = doc

    def create_tree(self, index: int, df: pd.DataFrame, path: str, parent: ET.Element) -> int:
        while index < df.shape[0]:
            if df.loc[index, 'Parent'] != path:
                break
            node = None
            if df.loc[index, 'NodeType'] == 'EquipmentStateClass':
                node = self.create_state_class(parent,df.iloc[index, :])

            if df.loc[index, 'NodeType'] == 'EquipmentState':
                node = self.create_state(parent, df.iloc[index, :])

            index = self.create_tree(index + 1, df, df.loc[index, 'Path'], node)
        return index    

    def create_state_class(self, parent: ET.Element, values: pd.DataFrame) -> ET.Element:
    ...
    def create_state(self, parent: ET.Element, values: pd.DataFrame) -> ET.Element:
...
if __name__ == '__main__':
    ...
    import_tool = StateImportTool(args.path)
    errors = import_tool.check_errors()
    if not errors:
        import_tool.generate_xml()
        import_tool.write_xml(name=args.name)